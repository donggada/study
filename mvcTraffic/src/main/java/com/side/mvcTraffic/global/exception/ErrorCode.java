...
public enum ErrorCode {
    ...
    INVALID_COUPON_ISSUE_QUANTITY(CONFLICT, BAD_REQUEST.toString(),"쿠폰 발급 수량이 유효하지 않습니다.","발급 가능한 수량을 초과합니다. total : %s, issued: %s"),
    INVALID_COUPON_ISSUE_DATE(CONFLICT, BAD_REQUEST.toString(),"쿠폰 발급 기간이 유효하지 않습니다.", ""),
    COUPON_NOT_EXIST(CONFLICT, BAD_REQUEST.toString(),"존재하지 않는 쿠폰입니다.", "쿠폰 정책이 존재하지 않습니다. %s"),
    DUPLICATED_COUPON_ISSUE(CONFLICT, BAD_REQUEST.toString(),"이미 발급된 쿠폰입니다.", "이미 발급된 쿠폰입니다. user_id: %d, coupon_id: %d"),
    ...
    public ApplicationException build() {
        return new ApplicationException(httpStatus, code, message, reason);
    }


    public ApplicationException build(Object ...args) {
        return new ApplicationException(httpStatus, code, message, reason.formatted(args));
    }
}
