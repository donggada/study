...
import com.side.mvcTraffic.global.exception.ErrorCode;
...
public enum CouponIssueRequestCode {
    ...
    public static void checkRequestResult(CouponIssueRequestCode code) throws ApplicationException {
        if (code == INVALID_COUPON_ISSUE_QUANTITY) {
            throw ErrorCode.INVALID_COUPON_ISSUE_QUANTITY.build();
        }
        if (code == DUPLICATED_COUPON_ISSUE) {
            throw ErrorCode.DUPLICATED_COUPON_ISSUE.build();
        }
    }
}
